...
function GatePass() {
    ...
    const submit = () => {
        ...
        // return;
        if (!leavePass.name || !leavePass.rollNo || !leavePass.destination || !leavePass.outTime || !leavePass.inDate || !leavePass.purpose) {
            window.alert("Please enter details correctly");
        }

        addDoc(collection(db, `pass/leavePass/${user?.email}`), { ...leavePass, date: serverTimestamp() })
            .then((result) => {
                window.alert("Leave Pass created successfully ! Check your email for confirmation.");
                async function sendMail() {
                    let data = await fetch('/api/leavepass', {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                        },
                        body: JSON.stringify({ ...leavePass, email: user.email }),
                    });
                        ...
                        <section class="text-gray-600 body-font relative">
                            <div class="container px-5 py-24 mx-auto">
                                <div class="flex flex-col text-center w-full mb-12">
                                    <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">Leave Pass</h1>
                                    ...
                                    <div class="flex flex-wrap -m-2">
                                        ...
                                        <div class="p-2 w-1/2">
                                            <div class="relative">
                                                ...
                                                <input type="email" id="email" onChange={onchange} name="inDate" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
                                            </div>
                                        </div>
